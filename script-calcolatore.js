...
function calculateProfit(){
  ...
  const mtaMens     = Math.max(0, num('speseMtaLicenza'));
  const extraFixedMens = [...document.querySelectorAll('[data-type="fixed-extra"]')]
    .reduce((s,i)=> s + Math.max(0, (+i.value||0)), 0);
  const utenzeMensili = lucegasMens + wifiMens + ammMens + acquaMens + mtaMens + extraFixedMens;
...
function saveToReport(model){
  ...
  try{
    localStorage.setItem('elevamalta:model', JSON.stringify(latestModel));
